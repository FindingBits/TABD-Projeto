...
DATABASE_URL = "postgresql://postgres:admin123@localhost:5432/eleicoes_db"
engine = create_engine(DATABASE_URL)
output = "data-output/data_output.sql"
...
# vem de outra fonte:    "parties"                       : [],   # (acronym,name)
#super dependência dos restantes     "elections"                     : [],   # (election_year,election_date,description)
#vai ter que esperar    "candidates"                    : [],   # (candidate_id,full_name)

#districts done
#municipalities done
#turnout done           ->output falta sql
#parties done   
#coalition_candidacies: falta o nome e expected mandates
#coalition_parties done ->output falta sql
...
#como só precisamos dos 2 primeiros para definir um distrito
#eliminamos o problema ao só extrair os 2 primeiros
#e preencher o resto com zeros como é suposto
def extract_district(df):
    ...
    for i,line in district_df.iterrows():
        ...
        if code < 100000:
            district_code = "0" + str(code)[0] + "0000"
        elif code < 300000:
        ...
        data["districts"].append((district_code,district_name,district_type))

    #print(data["districts"])

#Calheta (R.A.A.) oh pá...
#em 2021 o df assume como inteiro os códigos
#em 2025 já assume como string...
...
def extract_mandates(mdf,EXCEL_FILE,year):
    ...
    for (idx,line_mandates),i in zip(mandates.iterrows(),range(0,len(data["municipalities"]))):
        ...
        #print(f"Nome do Concelho: df: {concelho_man} / lista de municipios: {concelho_mun}")
    ...
    print("\t\tTurnover: Done")
    #print(len(dep_data["turnout"]) == len(data["municipalities"]))
    #print(f"turnout: {len(dep_data['turnout'])} municipalities:{len(data['municipalities'])} ")
    #print(dep_data["turnout"])
...
def extract_coalitions(df,year):
    ...
    # print(df)
    # print(df_mandates)
    # print(df[df['CONC']=="PORTO MONIZ"])
    # print(df_mandates[df_mandates['CONC']=="PORTO MONIZ"])
    # df = df[df['SIGLAS COLIGAÇÕES'].notna()]
    # print(df)
    # df = df.drop(df.columns[2:27], axis=1)
    # df = df.drop(df.columns[5:9], axis=1)
    print(df)
    print(df_mandates)
...
def extract_party_candidacies(df:pd.DataFrame,year):
    ...
    #df_mandates = df_mandates.drop(df_mandates.columns[2:start_i], axis=1)
    #df_mandates = df_mandates.drop(df_mandates.columns[24:], axis=1)
    headers = list(df_mandates.columns)
    ...
    #print(df_mandates[df_mandates["PS"].notna()])
    ...
    for (ind,line),(indx,line_m) in zip(df.iterrows(),df_mandates.iterrows()):
        ...
        for party in party_cols:
            if pd.notna(line[party]):
                ...
                #print(f"Concelho: {municipality_code} Partido: {party} votos: {votes} mandatos: {expected_mandates}")
